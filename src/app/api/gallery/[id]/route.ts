...
export async function PATCH(
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Gallery item status toggled", active: nextActive });
  } catch (error) {
    console.error("PATCH /api/gallery/[id] error:", error);
    return NextResponse.json({ success: false, message: "Failed to toggle gallery item status" }, { status: 500 });
  }
}
